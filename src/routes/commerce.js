...
import { checkRateLimit, createDownloadToken, ensureStorefront, formatCurrency, getClientIdentifier, queueEmail, sha256Hex, verifyDownloadToken, verifyWebhookSignature, } from '../lib/commerce';
...
function getPlatformFeePercent(value) {
...
commerce.post('/products/create', authMiddleware, async (c) => {
    try {
        ...
        const user = await c.env.DB.prepare('SELECT id, name, plan FROM users WHERE id = ?').bind(userId).first();
        if (!user) {
            return c.json(apiError('User not found'), 404);
        }
        const count = await c.env.DB.prepare('SELECT COUNT(*) as total FROM digital_products WHERE user_id = ?').bind(userId).first();
        ...
        await c.env.DB.prepare(`INSERT INTO digital_products (id, user_id, storefront_id, title, description, price, currency, delivery_type, thumbnail_url, delivery_link)
       VALUES (?, ?, ?, ?, ?, ?, 'usd', ?, ?, ?)`).bind(productId, userId, storefront.storefrontId, title, description, price, deliveryType, thumbnailUrl, deliveryLink).run();
        if (deliveryType === 'file') {
            await c.env.DB.prepare('INSERT INTO product_assets (id, product_id, file_name, mime_type, file_size, file_data_base64) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId('asset'), productId, fileName || `${title}.pdf`, fileType, Number(body.file_size || 0), fileDataBase64).run();
        }
        await c.env.DB.prepare('INSERT INTO notifications (id, user_id, title, message, type, link) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId('notif'), userId, 'Product created', `${title} is live in your storefront.`, 'success', '/app/products').run();
...
commerce.get('/products/mine', authMiddleware, async (c) => {
    const userId = c.get('userId');
    const products = await c.env.DB.prepare(`SELECT p.*, s.username,
...
commerce.get('/users/:id/products', async (c) => {
    ...
    const products = await c.env.DB.prepare(`SELECT id, title, description, price, currency, delivery_type, thumbnail_url, created_at
...
commerce.put('/products/:id', authMiddleware, async (c) => {
    const userId = c.get('userId');
    const productId = c.req.param('id');
    const existing = await c.env.DB.prepare('SELECT id, user_id, delivery_type FROM digital_products WHERE id = ? AND user_id = ?').bind(productId, userId).first();
    if (!existing) {
        return c.json(apiError('Product not found'), 404);
    }
    const body = await c.req.json();
    await c.env.DB.prepare(`UPDATE digital_products SET title = ?, description = ?, price = ?, thumbnail_url = ?, delivery_link = ?, updated_at = datetime('now')
     WHERE id = ? AND user_id = ?`).bind(body.title || '', body.description || '', Number(body.price || 0), body.thumbnail_url || '', body.delivery_link || '', productId, userId).run();
    if (existing.delivery_type === 'file' && body.file_data_base64) {
        await c.env.DB.prepare(`UPDATE product_assets SET file_name = ?, mime_type = ?, file_size = ?, file_data_base64 = ? WHERE product_id = ?`).bind(body.file_name || '', body.file_type || 'application/octet-stream', Number(body.file_size || 0), body.file_data_base64, productId).run();
    }
...
commerce.delete('/products/:id', authMiddleware, async (c) => {
    const userId = c.get('userId');
    const productId = c.req.param('id');
    const result = await c.env.DB.prepare('DELETE FROM digital_products WHERE id = ? AND user_id = ?').bind(productId, userId).run();
...
commerce.post('/products/:id/track', async (c) => {
    ...
    const product = await c.env.DB.prepare('SELECT id, user_id, title FROM digital_products WHERE id = ? AND is_published = 1').bind(productId).first();
    if (!product) {
        return c.json(apiError('Product not found'), 404);
    }
    await c.env.DB.prepare(`INSERT INTO product_events (id, product_id, seller_user_id, event_type, session_id, source_platform, source_post_id, metadata)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('pev'), productId, product.user_id, eventType, body.session_id || generateId('sess'), body.source_platform || '', body.source_post_id || '', JSON.stringify(body.metadata || {})).run();
    if (eventType === 'view') {
        const views = await c.env.DB.prepare(`SELECT COUNT(*) as total FROM product_events WHERE product_id = ? AND event_type = 'view'`).bind(productId).first();
        if ([25, 100].includes(Number(views?.total || 0))) {
            await c.env.DB.prepare('INSERT INTO notifications (id, user_id, title, message, type, link) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId('notif'), product.user_id, 'Your product is trending', `${product.title} crossed ${views.total} views. Push it with a fresh post today.`, 'info', '/app/products').run();
        }
    }
...
commerce.get('/analytics/product/:id', authMiddleware, async (c) => {
    const userId = c.get('userId');
    const productId = c.req.param('id');
    const product = await c.env.DB.prepare('SELECT id, title FROM digital_products WHERE id = ? AND user_id = ?').bind(productId, userId).first();
    if (!product) {
        return c.json(apiError('Product not found'), 404);
    }
    const [events, paidOrders, topProducts, weeklyOrders] = await Promise.all([
        c.env.DB.prepare(`SELECT event_type, COUNT(*) as total FROM product_events WHERE product_id = ? GROUP BY event_type`).bind(productId).all(),
        c.env.DB.prepare(`SELECT COUNT(*) as purchases, COALESCE(SUM(amount), 0) as revenue FROM commerce_orders WHERE product_id = ? AND payment_status = 'paid'`).bind(productId).first(),
        c.env.DB.prepare(`SELECT p.id, p.title, COUNT(o.id) as sales_count, COALESCE(SUM(o.amount), 0) as revenue
       FROM digital_products p
       ...
       LIMIT 5`).bind(userId).all(),
        c.env.DB.prepare(`SELECT DATE(created_at) as day, COUNT(*) as sales, COALESCE(SUM(amount), 0) as revenue
       FROM commerce_orders
       WHERE seller_user_id = ? AND payment_status = 'paid' AND created_at >= datetime('now', '-7 days')
       GROUP BY DATE(created_at)
       ORDER BY day ASC`).bind(userId).all(),
    ]);
    const counts = Object.fromEntries((events.results || []).map((row) => [row.event_type, Number(row.total || 0)]));
    const purchases = Number(paidOrders?.purchases || 0);
    ...
    return c.json(apiSuccess({
        product: {
            ...
            clicks,
            purchases,
            revenue: Number(paidOrders?.revenue || 0),
...
commerce.get('/reports/weekly-revenue', authMiddleware, async (c) => {
    ...
    const [orders, topProduct] = await Promise.all([
        c.env.DB.prepare(`SELECT COUNT(*) as sales, COALESCE(SUM(amount), 0) as revenue
       FROM commerce_orders
       WHERE seller_user_id = ? AND payment_status = 'paid' AND created_at >= datetime('now', '-7 days')`).bind(userId).first(),
        c.env.DB.prepare(`SELECT p.title, COUNT(o.id) as sales_count, COALESCE(SUM(o.amount), 0) as revenue
       FROM digital_products p
    ...
    const sales = Number(orders?.sales || 0);
    const suggestions = [
...
commerce.get('/ai/sales-suggestions', authMiddleware, async (c) => {
    const userId = c.get('userId');
    const topProduct = await c.env.DB.prepare(`SELECT p.id, p.title,
...
commerce.get('/storefront/:username', async (c) => {
    ...
    const store = await c.env.DB.prepare(`SELECT s.id, s.username, s.headline, s.bio, s.accent_color, u.id as user_id, u.name, u.avatar_url, u.niche
     FROM creator_storefronts s
     JOIN users u ON s.user_id = u.id
     WHERE s.username = ?`).bind(username).first();
    if (!store) {
        return c.json(apiError('Storefront not found'), 404);
    }
    const products = await c.env.DB.prepare(`SELECT id, title, description, price, currency, thumbnail_url, delivery_type
     FROM digital_products
     WHERE user_id = ? AND is_published = 1
     ORDER BY created_at DESC`).bind(store.user_id).all();
...
commerce.post('/payments/checkout', async (c) => {
    ...
    const product = await c.env.DB.prepare('SELECT id, user_id, title, price, currency, is_published FROM digital_products WHERE id = ?').bind(productId).first();
    ...
    await c.env.DB.prepare(`INSERT INTO commerce_orders (id, seller_user_id, product_id, buyer_name, buyer_email, amount, platform_fee_percent, platform_fee_amount, seller_net_amount, currency, payment_provider, external_order_id, source_platform, source_post_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(orderId, product.user_id, productId, buyerName, buyerEmail, product.price, platformFeePercent, platformFeeAmount, sellerNetAmount, product.currency || 'usd', provider, `${provider}_${orderId}`, sourcePlatform, sourcePostId).run();
    await c.env.DB.prepare(`INSERT INTO product_events (id, product_id, seller_user_id, order_id, event_type, visitor_email, source_platform, source_post_id, metadata)
     VALUES (?, ?, ?, ?, 'click', ?, ?, ?, ?)`).bind(generateId('pev'), productId, product.user_id, orderId, buyerEmail, sourcePlatform, sourcePostId, JSON.stringify({ provider })).run();
...
commerce.post('/payments/orders/:id/mark-paid', async (c) => {
    ...
    const order = await c.env.DB.prepare(`SELECT o.*, p.title, p.delivery_type, p.delivery_link, p.user_id as seller_id, u.name as seller_name
     FROM commerce_orders o
     JOIN digital_products p ON o.product_id = p.id
     JOIN users u ON p.user_id = u.id
     WHERE o.id = ?`).bind(orderId).first();
    if (!order) {
    ...
    await c.env.DB.prepare(`UPDATE commerce_orders SET payment_status = 'paid', delivery_status = 'ready', external_payment_id = ?, paid_at = datetime('now') WHERE id = ?`).bind(`demo_${orderId}`, orderId).run();
    await c.env.DB.prepare(`INSERT INTO product_download_tokens (id, order_id, product_id, token_hash, expires_at)
     VALUES (?, ?, ?, ?, ?)`).bind(generateId('dl'), orderId, order.product_id, tokenBundle.tokenHash, tokenBundle.expiresAt).run();
    await c.env.DB.prepare(`INSERT INTO product_events (id, product_id, seller_user_id, order_id, event_type, visitor_email, source_platform, source_post_id)
     VALUES (?, ?, ?, ?, 'purchase', ?, ?, ?)`).bind(generateId('pev'), order.product_id, order.seller_id, orderId, order.buyer_email, order.source_platform || '', order.source_post_id || '').run();
    await c.env.DB.prepare('INSERT INTO notifications (id, user_id, title, message, type, link) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId('notif'), order.seller_id, 'You made a sale', `${order.title} sold for ${formatCurrency(order.amount, order.currency)}.`, 'success', '/app/products').run();
    const downloadUrl = `/downloads/${tokenBundle.token}`;
    await queueEmail(c.env.DB, order.buyer_email, `Your purchase: ${order.title}`, `<h2>Thanks for your purchase</h2><p>You bought <strong>${order.title}</strong>.</p><p><a href="${downloadUrl}">Download your file</a></p><p>This link expires in 24 hours.</p>`, order.seller_id);
    await queueEmail(c.env.DB, order.buyer_email, `Receipt for ${order.title}`, `<p>Receipt</p><p>Amount: ${formatCurrency(order.amount, order.currency)}</p><p>Order ID: ${orderId}</p>`, order.seller_id);
...
commerce.get('/orders/:id', async (c) => {
    const orderId = c.req.param('id');
    const order = await c.env.DB.prepare(`SELECT o.id, o.buyer_email, o.amount, o.platform_fee_percent, o.platform_fee_amount, o.seller_net_amount, o.currency, o.payment_status, o.paid_at,
...
commerce.get('/downloads/:token', async (c) => {
    ...
    const dbToken = await c.env.DB.prepare(`SELECT d.*, o.payment_status, p.title, p.delivery_type, p.delivery_link,
    ...
    await c.env.DB.prepare(`UPDATE product_download_tokens SET download_count = download_count + 1, last_downloaded_at = datetime('now') WHERE id = ?`).bind(dbToken.id).run();
    await c.env.DB.prepare(`INSERT INTO product_events (id, product_id, seller_user_id, order_id, event_type, visitor_email)
     VALUES (?, ?, (SELECT seller_user_id FROM commerce_orders WHERE id = ?), ?, 'download', (SELECT buyer_email FROM commerce_orders WHERE id = ?))`).bind(generateId('pev'), dbToken.product_id, dbToken.order_id, dbToken.order_id, dbToken.order_id).run();
