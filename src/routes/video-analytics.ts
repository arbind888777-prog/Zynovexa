...
const videoAnalytics = new Hono<{ Bindings: Bindings; Variables: Variables }>();
...
videoAnalytics.get('/overview', async (c) => {
  ...
  const videoCount = await c.env.DB.prepare(
    "SELECT COUNT(*) as count FROM posts WHERE user_id = ? AND media_type = 'video'"
  ).bind(userId).first() as any;

  const videos = await c.env.DB.prepare(
    "SELECT * FROM posts WHERE user_id = ? AND media_type = 'video' ORDER BY created_at DESC LIMIT 20"
  ).bind(userId).all();

  // Fetch video-specific metrics from video_metadata table
  const videoMeta = await c.env.DB.prepare(
    'SELECT * FROM video_metadata WHERE user_id = ? ORDER BY created_at DESC LIMIT 20'
  ).bind(userId).all().catch(() => ({ results: [] }));

  // Aggregate stats
  const metaList = videoMeta.results || [];
  const totalViews = metaList.reduce((s: number, v: any) => s + (v.views || 0), 0);
  const totalWatchHours = metaList.reduce((s: number, v: any) => s + (v.watch_time_minutes || 0), 0) / 60;
  const avgRetention = metaList.length > 0 ? metaList.reduce((s: number, v: any) => s + (v.avg_retention_pct || 0), 0) / metaList.length : 0;
  const avgCTR = metaList.length > 0 ? metaList.reduce((s: number, v: any) => s + (v.ctr_pct || 0), 0) / metaList.length : 0;
  const totalSubsGained = metaList.reduce((s: number, v: any) => s + (v.subscribers_gained || 0), 0);
  const totalLikes = metaList.reduce((s: number, v: any) => s + (v.likes || 0), 0);
  const totalComments = metaList.reduce((s: number, v: any) => s + (v.comments || 0), 0);
...
videoAnalytics.get('/:postId', async (c) => {
  ...
  const post = await c.env.DB.prepare(
    "SELECT * FROM posts WHERE id = ? AND user_id = ? AND media_type = 'video'"
  ).bind(postId, userId).first();
  if (!post) return c.json(apiError('Video not found'), 404);

  const meta = await c.env.DB.prepare(
    'SELECT * FROM video_metadata WHERE post_id = ? AND user_id = ?'
  ).bind(postId, userId).first().catch(() => null);
...
videoAnalytics.post('/metadata', async (c) => {
  try {
    const userId = c.get('userId');
    const { post_id, title, description, tags, category, thumbnail_url, duration_seconds,
            video_url, visibility, seo_title, seo_description, seo_tags } = await c.req.json();
    
    if (!post_id) return c.json(apiError('post_id required'), 400);

    const existing = await c.env.DB.prepare(
      'SELECT id FROM video_metadata WHERE post_id = ? AND user_id = ?'
    ...
    if (existing) {
      ...
      `).bind(
        title || '', description || '', JSON.stringify(tags || []), category || '',
        thumbnail_url || '', duration_seconds || 0, video_url || '', visibility || 'public',
        seo_title || '', seo_description || '', JSON.stringify(seo_tags || []),
        post_id, userId
      ).run();
    } else {
      await c.env.DB.prepare(`
      ...
      `).bind(
        generateId('vmeta'), post_id, userId, title || '', description || '',
        JSON.stringify(tags || []), category || '', thumbnail_url || '',
        duration_seconds || 0, video_url || '', visibility || 'public',
        seo_title || '', seo_description || '', JSON.stringify(seo_tags || [])
      ).run();
    }
