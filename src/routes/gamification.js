...
const BADGE_DEFINITIONS = [
    { type: 'first_post', name: 'First Post', icon: '🎯', condition: (stats) => stats.total_posts >= 1 },
    { type: 'streak_7', name: '7-Day Streak', icon: '🔥', condition: (stats) => stats.current_streak >= 7 },
    { type: 'streak_30', name: '30-Day Streak', icon: '💎', condition: (stats) => stats.current_streak >= 30 },
    { type: 'streak_100', name: '100-Day Legend', icon: '👑', condition: (stats) => stats.current_streak >= 100 },
    { type: 'posts_10', name: 'Content Creator', icon: '✍️', condition: (stats) => stats.total_posts >= 10 },
    { type: 'posts_50', name: 'Prolific Publisher', icon: '📚', condition: (stats) => stats.total_posts >= 50 },
    { type: 'posts_100', name: 'Content Machine', icon: '🏭', condition: (stats) => stats.total_posts >= 100 },
    { type: 'viral_post', name: 'Gone Viral', icon: '🚀', condition: (stats) => stats.highest_viral_score >= 90 },
    { type: 'ai_power_user', name: 'AI Power User', icon: '🤖', condition: (stats) => stats.total_ai_uses >= 50 },
    { type: 'multi_platform', name: 'Multi-Platform Pro', icon: '🌐', condition: (stats) => stats.connected_accounts >= 3 },
    { type: 'engaged', name: 'Community Engager', icon: '💬', condition: (stats) => stats.total_engagement >= 1000 },
    { type: 'growth_10k', name: '10K Club', icon: '⭐', condition: (stats) => stats.total_followers >= 10000 },
];
...
gamification.get('/profile', async (c) => {
    ...
    let streak = await c.env.DB.prepare('SELECT * FROM user_streaks WHERE user_id = ?').bind(userId).first();
    if (!streak) {
    ...
    return c.json(apiSuccess({
        ...
        available_badges: BADGE_DEFINITIONS.map(b => ({
            ...
            name: b.name,
            icon: b.icon,
            earned: (badges.results || []).some((eb) => eb.badge_type === b.type),
        })),
        stats: {
...
gamification.get('/reminders', async (c) => {
    const userId = c.get('userId');
    const result = await c.env.DB.prepare("SELECT id, reminder_type, title, message, created_at FROM user_reminders WHERE user_id = ? AND sent = 0 ORDER BY created_at DESC LIMIT 10").bind(userId).all();
...
async function checkAndAwardBadges(c, userId) {
    // Get current stats
    ...
    const earned = new Set((existingBadges.results || []).map((b) => b.badge_type));
    const stats = {
        current_streak: streak?.current_streak || 0,
        total_posts: postCount?.count || 0,
        total_ai_uses: aiCount?.count || 0,
        connected_accounts: accountCount?.count || 0,
        highest_viral_score: topScore?.max_score || 0,
