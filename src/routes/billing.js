...
billing.get('/', async (c) => {
    ...
    const payments = await c.env.DB.prepare('SELECT * FROM payments WHERE user_id = ? ORDER BY created_at DESC LIMIT 20').bind(userId).all();
...
billing.post('/upgrade', async (c) => {
    try {
        ...
        await c.env.DB.prepare('UPDATE subscriptions SET plan = ?, current_period_start = datetime("now"), current_period_end = datetime("now", "+30 days"), updated_at = datetime("now") WHERE user_id = ?').bind(plan, userId).run();
        const prices = { free: 0, pro: 1900, business: 4900 };
        if (prices[plan] > 0) {
            await c.env.DB.prepare('INSERT INTO payments (id, user_id, amount, currency, status, description) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId('pay'), userId, prices[plan], 'usd', 'succeeded', `Upgrade to ${plan}`).run();
        }
