...
admin.get('/overview', async (c) => {
    ...
    const todayUsers = await c.env.DB.prepare("SELECT COUNT(*) as count FROM users WHERE created_at >= date('now')").first();
    const todayPosts = await c.env.DB.prepare("SELECT COUNT(*) as count FROM posts WHERE created_at >= date('now')").first();
    ...
    return c.json(apiSuccess({
        ...
        total_ai_requests: aiReqs?.count || 0,
        total_activity_logs: logs?.count || 0,
        today_signups: todayUsers?.count || 0,
        today_posts: todayPosts?.count || 0,
        total_revenue: revenue?.total || 0,
        subscriptions_by_plan: subs?.results || []
    }));
});
...
admin.get('/users', async (c) => {
    ...
    return c.json(apiSuccess({ users: result.results, total: total?.count || 0 }));
});
// GET /api/admin/users/:id - Get user details
admin.get('/users/:id', async (c) => {
    const userId = c.req.param('id');
    const user = await c.env.DB.prepare('SELECT id, email, name, role, niche, plan, onboarded, timezone, created_at FROM users WHERE id = ?').bind(userId).first();
    if (!user)
    ...
    return c.json(apiSuccess({ user, accounts: accounts.results, post_count: postCount?.count || 0, subscription: sub }));
});
// PUT /api/admin/users/:id - Update user (role, plan, etc.)
admin.put('/users/:id', async (c) => {
    const userId = c.req.param('id');
    const { role, plan, niche } = await c.req.json();
    await c.env.DB.prepare('UPDATE users SET role = COALESCE(?, role), plan = COALESCE(?, plan), niche = COALESCE(?, niche), updated_at = datetime("now") WHERE id = ?').bind(role || null, plan || null, niche || null, userId).run();
    if (plan) {
