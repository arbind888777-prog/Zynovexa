...
trust.get('/case-studies', async (c) => {
    const result = await c.env.DB.prepare('SELECT id, title, niche, duration_days, before_metrics, after_metrics, strategy_used, tools_used, growth_followers, growth_engagement, growth_revenue FROM case_studies WHERE published = 1 ORDER BY created_at DESC LIMIT 10').all();
    const studies = (result.results || []).map((s) => ({
        ...s,
        before_metrics: JSON.parse(s.before_metrics || '{}'),
        after_metrics: JSON.parse(s.after_metrics || '{}'),
        tools_used: JSON.parse(s.tools_used || '[]'),
        growth_percentage: s.before_metrics ? calculateGrowth(JSON.parse(s.before_metrics || '{}'), JSON.parse(s.after_metrics || '{}')) : null,
    }));
    return c.json(apiSuccess({ case_studies: studies }));
});
// ── PUBLIC: Get roadmap ─────────────────────────────────────
trust.get('/roadmap', async (c) => {
    const result = await c.env.DB.prepare('SELECT id, title, description, category, status, votes, target_date, released_at FROM roadmap_items ORDER BY priority DESC, votes DESC').all();
