...
monetization.get('/overview', async (c) => {
    ...
    const deals = await c.env.DB.prepare('SELECT * FROM brand_deals WHERE user_id = ? ORDER BY created_at DESC').bind(userId).all();
    const totalEarnings = await c.env.DB.prepare("SELECT SUM(deal_value) as total FROM brand_deals WHERE user_id = ? AND status IN ('completed', 'in_progress')").bind(userId).first();
    const pendingDeals = await c.env.DB.prepare("SELECT COUNT(*) as count FROM brand_deals WHERE user_id = ? AND status IN ('negotiating', 'confirmed')").bind(userId).first();
    const accounts = await c.env.DB.prepare('SELECT platform, followers_count FROM accounts WHERE user_id = ?').bind(userId).all();
    const totalFollowers = (accounts.results || []).reduce((sum, a) => sum + a.followers_count, 0);
    ...
    return c.json(apiSuccess({
        ...
        pending_deals: pendingDeals?.count || 0,
        deals: deals.results,
        rate_estimate: {
...
monetization.post('/deals', async (c) => {
    try {
        ...
        if (!brand_name)
            return c.json(apiError('Brand name is required'), 400);
...
monetization.get('/media-kit', async (c) => {
    const userId = c.get('userId');
    const user = await c.env.DB.prepare('SELECT name, niche, avatar_url FROM users WHERE id = ?').bind(userId).first();
    ...
    return c.json(apiSuccess({
        ...
        stats: {
            ...
            completed_partnerships: completedDeals?.count || 0,
