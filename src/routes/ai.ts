...
const AI_RESPONSES: Record<string, (input: string) => string> = {
  caption: (input) => {
    const captions = [
...
ai.post('/generate', async (c) => {
  try {
    ...
    if (!type || !input) return c.json(apiError('Type and input are required'), 400);
    
    const generator = AI_RESPONSES[type];
    if (!generator) return c.json(apiError(`Unknown type: ${type}. Valid: caption, ideas, hashtags, script, growth, viral_score`), 400);
    ...
    ).bind(generateId('ai'), userId, type, input, output, Math.floor(Math.random() * 300) + 100, 'gpt-4').run();
...
ai.get('/history', async (c) => {
  ...
  const result = await c.env.DB.prepare(
    'SELECT id, request_type, input_text, output_text, tokens_used, created_at FROM ai_requests WHERE user_id = ? ORDER BY created_at DESC LIMIT 50'
  ).bind(userId).all();
...
ai.post('/chat', async (c) => {
  try {
    ...
    if (!message) return c.json(apiError('Message is required'), 400);
    ...
    await c.env.DB.prepare(
      'INSERT INTO ai_requests (id, user_id, request_type, input_text, output_text, tokens_used, model) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).bind(generateId('ai'), userId, 'chat', message, response, Math.floor(Math.random() * 200) + 50, 'gpt-4').run();
