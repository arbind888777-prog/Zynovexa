...
seo.post('/analyze', async (c) => {
    try {
        ...
        if (!title)
            return c.json(apiError('Title is required'), 400);
        // === SEO Scoring Engine ===
        let score = 0;
        const suggestions = [];
        const checks = [];
        ...
        if (titleLen >= 40 && titleLen <= 70) {
            score += 20;
            checks.push({ label: 'Title Length', pass: true, detail: titleLen + ' chars — optimal range (40-70)' });
        }
        else if (titleLen > 0 && titleLen < 40) {
            score += 10;
            checks.push({ label: 'Title Length', pass: false, detail: titleLen + ' chars — too short, aim for 40-70' });
            suggestions.push('Lengthen your title to 40-70 characters for better search ranking.');
        }
        else if (titleLen > 70) {
            score += 8;
            checks.push({ label: 'Title Length', pass: false, detail: titleLen + ' chars — will be truncated in search results' });
        ...
        if (hasNumber || hasPowerWord) {
            score += 15;
            checks.push({ label: 'Title Power', pass: true, detail: 'Contains ' + (hasNumber ? 'numbers' : '') + (hasNumber && hasPowerWord ? ' & ' : '') + (hasPowerWord ? 'power words' : '') });
        }
        else {
            score += 5;
            checks.push({ label: 'Title Power', pass: false, detail: 'No numbers or power words found' });
        ...
        if (descLen >= 200) {
            score += 20;
            checks.push({ label: 'Description', pass: true, detail: descLen + ' chars — good detail' });
        }
        else if (descLen >= 50) {
            score += 10;
            checks.push({ label: 'Description', pass: false, detail: descLen + ' chars — add more detail (aim for 200+)' });
            suggestions.push('Expand your description to 200+ characters. Include keywords naturally in the first 2 lines.');
        }
        else {
            score += 0;
            checks.push({ label: 'Description', pass: false, detail: descLen + ' chars — too short or empty' });
        ...
        if (hasTimestamps) {
            score += 5;
            checks.push({ label: 'Timestamps', pass: true, detail: 'Timestamps found — helps YouTube create chapters' });
        }
        else {
        ...
        if (tagList.length >= 5 && tagList.length <= 15) {
            score += 15;
            checks.push({ label: 'Tags', pass: true, detail: tagList.length + ' tags — optimal (5-15)' });
        }
        else if (tagList.length >= 1) {
            score += 8;
            checks.push({ label: 'Tags', pass: false, detail: tagList.length + ' tags — ' + (tagList.length < 5 ? 'add more' : 'too many, keep under 15') });
        ...
        const keywordMatch = titleWords.some((w) => descLower.includes(w));
        if (keywordMatch && descLen > 50) {
            score += 10;
            checks.push({ label: 'Keyword Match', pass: true, detail: 'Title keywords found in description' });
        }
        else {
            score += 3;
            checks.push({ label: 'Keyword Match', pass: false, detail: 'Title keywords not found in description' });
        ...
        if (firstLine.length >= 50 && firstLine.length <= 160) {
            score += 10;
            checks.push({ label: 'Hook Line', pass: true, detail: 'First line has good length for search preview' });
        }
        else {
            score += 3;
            checks.push({ label: 'Hook Line', pass: false, detail: 'First line should be 50-160 chars (search preview)' });
        ...
        if (hasCTA) {
            score += 5;
            checks.push({ label: 'Call to Action', pass: true, detail: 'CTA found in description' });
        }
        else {
        ...
        // Log AI usage
        await c.env.DB.prepare('INSERT INTO ai_requests (id, user_id, request_type, input_text, output_text, tokens_used, model) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(generateId('ai'), userId, 'seo_analyze', title, JSON.stringify({ score, grade }), 50, 'seo-engine').run();
...
seo.post('/keywords', async (c) => {
    try {
        ...
        if (!topic)
            return c.json(apiError('Topic is required'), 400);
        const topicLower = topic.toLowerCase();
        // Simulated keyword research engine (production: YouTube Data API + Google Trends)
        const baseKeywords = topicLower.split(/\s+/).filter((w) => w.length > 2);
        const keywords = [
        ...
        await c.env.DB.prepare('INSERT INTO ai_requests (id, user_id, request_type, input_text, output_text, tokens_used, model) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(generateId('ai'), userId, 'seo_keywords', topic, JSON.stringify(keywords.slice(0, 3)), 80, 'seo-engine').run();
...
seo.post('/generate-tags', async (c) => {
    try {
        ...
        // Individual words
        unique.slice(0, 5).forEach((w) => tags.push(w));
        // Combinations
        if (unique.length >= 2)
            tags.push(unique[0] + ' ' + unique[1]);
        if (unique.length >= 3)
            tags.push(unique[1] + ' ' + unique[2]);
        // Common additions
        const additions = ['tutorial', 'how to', '2026', 'tips', 'guide', 'beginner', 'explained'];
        additions.forEach(a => { if (!tags.some(t => t.includes(a)))
            tags.push(unique[0] + ' ' + a); });
