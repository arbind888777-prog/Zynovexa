...
aiEngine.post('/generate', async (c) => {
    ...
    const todayUsage = await c.env.DB.prepare("SELECT COUNT(*) as count FROM ai_requests WHERE user_id = ? AND created_at >= date('now')").bind(userId).first();
    const limits = { free: 10, pro: 100, promax: 500, business: 9999 };
    ...
    await c.env.DB.prepare('INSERT INTO ai_requests (id, user_id, request_type, input_text, output_text, tokens_used, model) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(requestId, userId, params.type, params.input, output, output.length, 'zynovexa-ai-v2').run();
    // Save content score
    await c.env.DB.prepare('INSERT INTO content_scores (id, user_id, content_text, overall_score, hook_score, readability_score, engagement_score, suggestions) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(generateId('cs'), userId, output, score.overall, score.hook, score.readability, score.engagement, JSON.stringify(score.suggestions)).run();
...
aiEngine.post('/score', async (c) => {
    ...
    if (!content)
        return c.json(apiError('Content is required'), 400);
    const score = scoreContent(content, type || 'caption');
    await c.env.DB.prepare('INSERT INTO content_scores (id, user_id, content_text, overall_score, hook_score, readability_score, engagement_score, suggestions) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(generateId('cs'), userId, content, score.overall, score.hook, score.readability, score.engagement, JSON.stringify(score.suggestions)).run();
    return c.json(apiSuccess({ score }));
});
// ════════════════════════════════════════════════════════════
// CONTENT GENERATION FUNCTIONS
// ════════════════════════════════════════════════════════════
function generateViralReelCaption(input, platform, audience, tone, niche) {
    const hooks = {
...
function generateYouTubeScript(input, audience, tone, niche) {
    const hooks = {
...
function generateHooks(input, platform, tone, niche) {
    const platformHooks = {
        instagram: [
...
function generateVideoScript(input, platform, audience, tone, niche) {
...
function generateNicheCaption(input, platform, audience, tone, niche) {
    const toneTemplates = {
...
function generateSmartHashtags(input, platform, niche) {
...
// ════════════════════════════════════════════════════════════
function scoreContent(content, type) {
    const suggestions = [];
    ...
    if (firstLine.length > 10)
        hookScore += 10;
    if (/stop|wait|pov|secret|truth|wrong|mistake|hack|nobody/i.test(firstLine))
        hookScore += 15;
    if (/\?/.test(firstLine))
        hookScore += 10;
    if (/[🔥💡👀🚀✨😂❤️⭐]/.test(firstLine))
    ...
    if (firstLine.length < 60)
        hookScore += 10; // Concise hooks perform better
    if (hookScore < 60)
        suggestions.push('Strengthen your opening hook — use curiosity, a question, or a bold statement');
    ...
    if (avgLineLength < 80)
        readabilityScore += 10;
    if (content.length > 50 && content.length < 2200)
        readabilityScore += 10;
    if (readabilityScore < 60)
        suggestions.push('Improve readability by using shorter paragraphs, bullet points, or numbered lists');
    readabilityScore = Math.min(100, readabilityScore);
    // Engagement score (0-100): CTA and interaction potential
    let engagementScore = 40;
    if (/\?/.test(content))
        engagementScore += 15; // Has questions
    if (/save|share|tag|follow|comment|like|double tap|bookmark/i.test(content))
        engagementScore += 15;
    if (/dm|reply|tell me|let me know|what do you think/i.test(content))
        engagementScore += 10;
    if (/#\w+/.test(content))
        engagementScore += 5;
    if (/👇|↓|below|thread/i.test(content))
        engagementScore += 5;
    if (/you|your|you're/i.test(content))
        engagementScore += 10; // Direct address
    if (engagementScore < 60)
        suggestions.push('Add a clear call-to-action (question, "save this", "share with someone")');
    engagementScore = Math.min(100, engagementScore);
    // Overall: Weighted average
    const overall = Math.round(hookScore * 0.4 + readabilityScore * 0.3 + engagementScore * 0.3);
    if (overall >= 80)
        suggestions.unshift('Great content! Minor tweaks could push this to viral territory.');
    else if (overall >= 60)
        suggestions.unshift('Solid content. Focus on the suggestions below to improve performance.');
