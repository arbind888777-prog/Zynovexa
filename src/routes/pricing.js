...
pricing.get('/my-plan', authMiddleware, async (c) => {
    ...
    return c.json(apiSuccess({
        ...
        usage: {
            ai_today: { used: aiUsage?.c || 0, limit: plan.limits.ai_daily },
            scheduled_posts: { used: postCount?.c || 0, limit: plan.limits.scheduled_posts },
            connected_accounts: { used: accountCount?.c || 0, limit: plan.limits.connected_accounts },
        },
    }));
});
...
pricing.post('/upgrade', authMiddleware, async (c) => {
    ...
    const currentUser = await c.env.DB.prepare('SELECT plan FROM users WHERE id = ?').bind(userId).first();
  ...
  `).bind(generateId('sub'), userId, plan, plan).run();
    // Log payment
    await c.env.DB.prepare('INSERT INTO payments (id, user_id, amount, currency, status, description) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId('pay'), userId, price * 100, 'usd', 'succeeded', `Upgrade to ${config.name}`).run();
    // Create notification
    await c.env.DB.prepare('INSERT INTO notifications (id, user_id, title, message, type) VALUES (?, ?, ?, ?, ?)').bind(generateId('notif'), userId, 'Plan Upgraded! 🎉', `Welcome to ${config.name}! All ${config.name} features are now unlocked.`, 'success').run();
    return c.json(apiSuccess({
        plan: { id: plan, ...config },
...
export function requireLimit(limitKey) {
    return async (c, next) => {
        ...
        if (limitKey === 'ai_daily') {
            const result = await c.env.DB.prepare("SELECT COUNT(*) as c FROM ai_requests WHERE user_id = ? AND created_at >= date('now')").bind(userId).first();
            currentUsage = result?.c || 0;
        }
        else if (limitKey === 'connected_accounts') {
            const result = await c.env.DB.prepare('SELECT COUNT(*) as c FROM accounts WHERE user_id = ?').bind(userId).first();
            currentUsage = result?.c || 0;
        }
        else if (limitKey === 'competitors') {
            const result = await c.env.DB.prepare('SELECT COUNT(*) as c FROM competitor_profiles WHERE user_id = ?').bind(userId).first();
            currentUsage = result?.c || 0;
        }
...
pricing.get('/nudge', authMiddleware, async (c) => {
    ...
    const user = await c.env.DB.prepare('SELECT plan, name FROM users WHERE id = ?').bind(userId).first();
    const planKey = user?.plan || 'free';
