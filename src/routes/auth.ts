...
auth.post('/signup', async (c) => {
  try {
    ...
    ).bind(userId, email.toLowerCase(), name, passwordHash, 'email').run();

    await c.env.DB.prepare(
      'INSERT INTO subscriptions (id, user_id, plan, status, current_period_start) VALUES (?, ?, ?, ?, ?)'
    ).bind(generateId('sub'), userId, 'free', 'active', new Date().toISOString()).run();
...
auth.post('/login', async (c) => {
  try {
    ...
    const user: any = await c.env.DB.prepare(
      'SELECT id, email, name, password_hash, role, niche, onboarded, plan, avatar_url, timezone FROM users WHERE email = ?'
...
auth.get('/me', async (c) => {
  ...
  if (!token) return c.json(apiError('No session'), 401);
  
  const { verifyToken } = await import('../lib/utils');
  const payload = await verifyToken(token, JWT_SECRET);
  if (!payload) return c.json(apiError('Invalid session'), 401);

  const user: any = await c.env.DB.prepare(
    'SELECT id, email, name, role, niche, onboarded, plan, avatar_url, timezone FROM users WHERE id = ?'
...
auth.post('/onboard', async (c) => {
  try {
    ...
    const { verifyToken } = await import('../lib/utils');
    const payload = await verifyToken(token, JWT_SECRET);
    if (!payload) return c.json(apiError('Auth required'), 401);

    const { niche, goals, plan } = await c.req.json();
    await c.env.DB.prepare(
      'UPDATE users SET niche = ?, onboarded = 1, plan = ?, updated_at = datetime("now") WHERE id = ?'
    ).bind(niche || '', plan || 'free', payload.userId).run();

    if (plan && plan !== 'free') {
      await c.env.DB.prepare(
        'UPDATE subscriptions SET plan = ?, updated_at = datetime("now") WHERE user_id = ?'
