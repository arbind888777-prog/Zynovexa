...
accounts.get('/', async (c) => {
  ...
  const result = await c.env.DB.prepare(
    'SELECT id, platform, platform_username, avatar_url, followers_count, status, created_at FROM accounts WHERE user_id = ? ORDER BY platform'
  ).bind(userId).all();
...
accounts.post('/connect', async (c) => {
  try {
    ...
    if (!platform) return c.json(apiError('Platform is required'), 400);
    
    const supported = ['instagram', 'facebook', 'youtube', 'linkedin', 'twitter', 'tiktok'];
    if (!supported.includes(platform)) return c.json(apiError(`Unsupported platform: ${platform}`), 400);

    // Check if already connected
    const existing = await c.env.DB.prepare(
      'SELECT id FROM accounts WHERE user_id = ? AND platform = ?'
    ).bind(userId, platform).first();
    if (existing) return c.json(apiError(`${platform} is already connected`), 409);
...
accounts.delete('/:id', async (c) => {
  ...
  if (!result.meta.changes) return c.json(apiError('Account not found'), 404);
...
accounts.post('/:id/reconnect', async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE accounts SET status = 'connected', updated_at = datetime('now') WHERE id = ? AND user_id = ?"
