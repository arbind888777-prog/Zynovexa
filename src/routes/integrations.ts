...
integrations.get('/platforms', async (c) => {
  ...
  const connected = await c.env.DB.prepare(
    'SELECT platform, status FROM accounts WHERE user_id = ?'
  ).bind(userId).all();

  const connectedMap: Record<string, string> = {};
  (connected.results || []).forEach((a: any) => { connectedMap[a.platform] = a.status; });
...
integrations.post('/refresh-token/:platform', async (c) => {
  ...
  const account = await c.env.DB.prepare(
    'SELECT id, refresh_token, token_expires_at FROM accounts WHERE user_id = ? AND platform = ?'
  ).bind(userId, platform).first() as any;
  ...
  ).bind(newToken, newExpiry, account.id).run();
...
integrations.delete('/queue/:id', async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE scheduled_jobs SET status = 'cancelled' WHERE id = ? AND user_id = ? AND status = 'pending'"
  ).bind(id, userId).run();
