...
proAnalytics.get('/overview', requireProPlan, async (c) => {
    ...
    const m7 = {};
    (analytics7d.results || []).forEach((r) => { m7[r.metric_type] = r.total; });
    const m30 = {};
    (analytics30d.results || []).forEach((r) => { m30[r.metric_type] = r.total; });
    const totalFollowersNum = totalFollowers?.total || 0;
  ...
  `).bind(userId).all();
    ...
    const latestFollowers = fh.length > 0 ? fh[0].metric_value : totalFollowersNum;
    const earlierFollowers = fh.length > 7 ? fh[7].metric_value : latestFollowers;
    ...
    return c.json(apiSuccess({
        metrics: {
            ...
            avg_likes_per_post: { value: postsCount?.count > 0 ? Math.round(likes7d / Math.max(postsCount.count, 1)) : 0, label: 'Avg Likes/Post' },
        },
        trends: {
...
proAnalytics.post('/competitors', requireProPlan, async (c) => {
    ...
    const count = await c.env.DB.prepare('SELECT COUNT(*) as c FROM competitor_profiles WHERE user_id = ?').bind(userId).first();
    if (count && count.c >= 5) {
...
proAnalytics.get('/competitors', requireProPlan, async (c) => {
  ...
  `).bind(userId).all();
    ...
    const userFollowers = await c.env.DB.prepare('SELECT SUM(followers_count) as total FROM accounts WHERE user_id = ?').bind(userId).first();
    const userAnalytics = await c.env.DB.prepare(`
    SELECT metric_type, SUM(metric_value) as total FROM analytics
    WHERE user_id = ? AND recorded_date >= date('now', '-7 days') GROUP BY metric_type
  `).bind(userId).all();
    const um = {};
    (userAnalytics.results || []).forEach((r) => { um[r.metric_type] = r.total; });
    ...
    return c.json(apiSuccess({
        ...
        comparison_insights: generateComparisonInsights(userFollowers?.total || 0, userEngagement, competitors.results || []),
    }));
});
...
// ── Helper functions ────────────────────────────────────────
async function getMetricTrend(c, userId, metricType, days) {
...
function generateContentInsights(topPosts, worstPosts) {
    const insights = [];
...
function generateComparisonInsights(userFollowers, userEngagement, competitors) {
    const insights = [];
    if (competitors.length === 0)
        return ['Add competitors to get comparison insights.'];
    const avgCompFollowers = competitors.reduce((s, c) => s + (c.followers_count || 0), 0) / competitors.length;
    const avgCompEngagement = competitors.reduce((s, c) => s + parseFloat(c.avg_engagement_rate || '0'), 0) / competitors.length;
