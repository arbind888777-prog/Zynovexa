...
const growthCoach = new Hono<{ Bindings: Bindings; Variables: Variables }>();
...
growthCoach.get('/daily', async (c) => {
  ...
  const niche = (userInfo as any)?.niche || 'general';
  const posts = recentPosts.results || [];
  const connectedPlatforms = (accounts.results || []).map((a: any) => a.platform);
  const streak = streakInfo as any;
  ...
  if (posts.length > 0) {
    const avgScore = posts.reduce((s: number, p: any) => s + (p.viral_score || 0), 0) / posts.length;
    if (avgScore < 60) {
      recommendations.push({
        type: 'improvement',
        icon: '📈',
...
growthCoach.get('/weekly-report', async (c) => {
  ...
  (analyticsThisWeek.results || []).forEach((r: any) => { metrics[r.metric_type] = r.total; });
  ...
  const postCount = (postsThisWeek as any)?.count || 0;
  if (postCount < 3) suggestions.push('Post at least 5 times per week for optimal growth');
  ...
  const report = {
    ...
    week_over_week: lastWeekReport ? {
      posts_change: postCount - ((lastWeekReport as any).total_posts || 0),
      engagement_change: totalEngagement - ((lastWeekReport as any).total_engagement || 0),
    } : null,
...
function generateContentIdeas(niche: string, platforms: string[], recentPosts: any[]) {
...
function getBestPostingTime(niche: string) {
...
function getAlgorithmInsight(niche: string, platforms: string[]) {
  const insights = [
...
function generateWeeklySummary(posts: number, impressions: number, engagement: number, rate: number) {
  if (posts === 0) return 'You didn\'t post this week. Consistency is key — try to post at least 3 times next week.';
  if (rate > 5) return `Great week! ${posts} posts with ${rate.toFixed(1)}% engagement rate. Your content is resonating well. Keep experimenting.`;
  if (rate > 2) return `Solid week with ${posts} posts and ${engagement} total engagements. Try adding stronger hooks to push past 5% engagement.`;
