...
posts.get('/', async (c) => {
  ...
  const countResult = await c.env.DB.prepare(
    'SELECT COUNT(*) as total FROM posts WHERE user_id = ?' + (status ? ' AND status = ?' : '')
  ).bind(...(status ? [userId, status] : [userId])).first();
  
  return c.json(apiSuccess({ posts: result.results, total: (countResult as any)?.total || 0 }));
});
...
posts.get('/:id', async (c) => {
  ...
  if (!post) return c.json(apiError('Post not found'), 404);
...
posts.post('/', async (c) => {
  try {
    ...
    if (attached_product_id) {
      attachedProduct = await c.env.DB.prepare(
        `SELECT p.id, p.title, s.username
         FROM digital_products p
         LEFT JOIN creator_storefronts s ON p.storefront_id = s.id
         WHERE p.id = ? AND p.user_id = ?`
      ).bind(attached_product_id, userId).first();
      if (!attachedProduct) {
    ...
    `).bind(
      postId, userId,
      finalCaption, JSON.stringify(media_urls || []),
      media_type || 'text', JSON.stringify(platforms || []),
      status || 'draft', scheduled_at || '', viralScore,
      JSON.stringify(hashtags || [])
    ).run();

    if (attachedProduct) {
      await c.env.DB.prepare(
        'INSERT INTO post_product_links (id, post_id, user_id, product_id, cta_text) VALUES (?, ?, ?, ?, ?)'
      ).bind(generateId('ppl'), postId, userId, attachedProduct.id, 'Buy here').run();
    }
...
posts.put('/:id', async (c) => {
  try {
    ...
    if (!existing) return c.json(apiError('Post not found'), 404);

    const { caption, media_urls, media_type, platforms, status, scheduled_at, hashtags, attached_product_id } = await c.req.json();
    let finalCaption = caption ?? (existing as any).caption;
    ...
    if (attached_product_id) {
      const attachedProduct = await c.env.DB.prepare(
        `SELECT p.id, s.username
         FROM digital_products p
         LEFT JOIN creator_storefronts s ON p.storefront_id = s.id
         WHERE p.id = ? AND p.user_id = ?`
      ).bind(attached_product_id, userId).first() as any;
      if (!attachedProduct) {
        return c.json(apiError('Attached product not found'), 404);
      }
      const ctaUrl = attachedProduct.username ? `/${attachedProduct.username}` : `/checkout/${attached_product_id}`;
    ...
    `).bind(
      finalCaption,
      media_urls ? JSON.stringify(media_urls) : (existing as any).media_urls,
      media_type ?? (existing as any).media_type,
      platforms ? JSON.stringify(platforms) : (existing as any).platforms,
      status ?? (existing as any).status,
      scheduled_at ?? (existing as any).scheduled_at,
      hashtags ? JSON.stringify(hashtags) : (existing as any).hashtags,
      postId, userId
    ).run();
...
posts.delete('/:id', async (c) => {
  ...
  if (!result.meta.changes) return c.json(apiError('Post not found'), 404);
...
posts.post('/:id/publish', async (c) => {
  const userId = c.get('userId');
  const postId = c.req.param('id');
  const post: any = await c.env.DB.prepare('SELECT * FROM posts WHERE id = ? AND user_id = ?').bind(postId, userId).first();
  if (!post) return c.json(apiError('Post not found'), 404);
  ...
  await c.env.DB.prepare(
    "UPDATE posts SET status = 'published', published_at = datetime('now'), publish_results = ?, updated_at = datetime('now') WHERE id = ?"
  ).bind(JSON.stringify(results), postId).run();
  ...
  for (const p of platforms) {
    ...
    for (const m of metrics) {
      await c.env.DB.prepare(
        'INSERT INTO analytics (id, user_id, post_id, platform, metric_type, metric_value, recorded_date) VALUES (?, ?, ?, ?, ?, ?, ?)'
      ).bind(generateId('an'), userId, postId, p, m, Math.floor(Math.random() * 1000) + 100, new Date().toISOString().split('T')[0]).run();
    }
  }
