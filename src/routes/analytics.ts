...
analytics.get('/dashboard', async (c) => {
  ...
  const totalFollowers = await c.env.DB.prepare(
    'SELECT SUM(followers_count) as total FROM accounts WHERE user_id = ?'
  ).bind(userId).first();
  ...
  (recentAnalytics.results || []).forEach((r: any) => { metricsMap[r.metric_type] = r.total; });

  const upcomingPosts = await c.env.DB.prepare(
    "SELECT id, caption, platforms, scheduled_at FROM posts WHERE user_id = ? AND status = 'scheduled' ORDER BY scheduled_at ASC LIMIT 5"
  ).bind(userId).all();

  return c.json(apiSuccess({
    stats: {
      total_posts: (totalPosts as any)?.count || 0,
      scheduled_posts: (scheduledPosts as any)?.count || 0,
      published_posts: (publishedPosts as any)?.count || 0,
      connected_accounts: (totalAccounts as any)?.count || 0,
      total_followers: (totalFollowers as any)?.total || 0,
      unread_notifications: (notifications as any)?.count || 0,
...
analytics.get('/heatmap', async (c) => {
  ...
  for (let d = 0; d < 7; d++) {
    for (let h = 0; h < 24; h++) {
      ...
      if (h >= 18 && h <= 21) value *= 2;
      if (h >= 12 && h <= 14) value *= 1.5;
      if (d >= 5) value *= 1.3;
